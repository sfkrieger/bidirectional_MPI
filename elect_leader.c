...
//			printf("Got a reply message... not dealing with it right now\n");
			...
			if (replies_rcvd[result.phase] == 2) {
				printf("Process %d aparently won phase %d (really %d). It had a distance of %d\n", world_rank, result.phase, to_pass.phase, result.hops);
				to_pass.phase = to_pass.phase + 1;
				to_pass.hops = 0;

				MPI_Send(&to_pass, 1, mpi_pkg, right, ELECTION, MPI_COMM_WORLD);
				MPI_Send(&to_pass, 1, mpi_pkg, left, ELECTION, MPI_COMM_WORLD);
				printf("Process %d sent out a new election for phase %d. The status of the counter at this phase is currently %d\n", world_rank, to_pass.phase, replies_rcvd[to_pass.phase]);
				no_sent = no_sent + 2;
			}

			if(reply_status == -1){
				printf("========== ERROR - GOT BAD REPLY STATUS IN PROCESS %d ===========", world_rank);
			}
...
//	int distance = ((primary_pkg->hops == 0) ? 1 : (1 << primary_pkg->hops));
	...
	if ((primary_pkg->id > id) && (distance < max_dist)) {
		...
		printf(
				...
				world_rank, id, primary_pkg->id, distance, max_dist, max, secondary);
	...
	if ((primary_pkg->id > id) && (distance == max_dist)) {
		printf(
				...
				world_rank, id, primary_pkg->id, distance, max_dist, max, primary);
...
//	printf("In reply message...\n");
	...
	if (id != primary_pkg->id) {
		...
		printf("Process %d got its own id back for phase %d. That brings the toll to %d\n", world_rank, primary_pkg->phase, temp);
		return 1;
	}
